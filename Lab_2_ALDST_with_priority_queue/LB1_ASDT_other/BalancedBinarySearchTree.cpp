...
TNode* BalancedBinarySearchTree::insert(TNode* p, Item k) // вставка ключа k в дерево с корнем p
{
	if(!p)
		return new TNode(k.Value, k.Priority);
	if(k.Priority < p->data.Priority)
...
TNode* BalancedBinarySearchTree::remove(TNode* p, Item k) // удаление ключа k из дерева p
{
	if(!p) return 0;
	if(k.Priority < p->data.Priority)
		p->left = remove(p->left, k);
	else if(k.Priority > p->data.Priority)
