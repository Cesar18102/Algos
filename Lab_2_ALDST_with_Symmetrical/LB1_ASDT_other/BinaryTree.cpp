...
BinaryTree BinaryTree::SymmetricalBBST()
{
	BinaryTree C;
	C.root = SymmetricalBBST_rec(root, C.root);
	return C;
}

TNode* BinaryTree::SymmetricalBBST_rec(TNode* rootOrig, TNode* rootCopy)
{
	if(rootOrig == NULL)
		return NULL;

	rootCopy = new TNode(rootOrig->data);
	rootCopy->left = SymmetricalBBST_rec(rootOrig->right, rootOrig->left);
	rootCopy->right = SymmetricalBBST_rec(rootOrig->left, rootOrig->right);
